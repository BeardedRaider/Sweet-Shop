<div class="bg-white rounded-lg shadow-md hover:shadow-lg transition p-4 text-center border border-pink-100">
    ...
    >
        ...
        {{-- Product name --}}
        <h3 class="mt-4 text-lg font-semibold text-pink-700">
            {{ $product->name }}
        </h3>
    </button>

    {{-- Short description --}}
    <p class="text-sm text-pink-900 mt-1">
    ...
    @auth
        <form action="{{ route('cart.add', $product->id) }}" method="POST"
            ...
            <div class="flex items-center gap-4">
                ...
                {{-- Display --}}
                <span
                    class="flex-none w-12 h-12 grid place-items-center bg-white border border-pink-300 rounded font-semibold text-pink-800 text-sm tabular-nums select-none overflow-hidden">
                    1
                </span>
